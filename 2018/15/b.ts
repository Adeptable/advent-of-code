...
utils.readLines((data: string[]) => {
    ...
    class unit {
        ...
        get attackPow(): number { return NaN }
    ...
    class elf extends unit {
        private _pow: number;
        constructor(p: pos, pow: number) {
            super(p);
            this._pow = pow
        }
        get symbol() { return 'E' }
        get attackPow() { return this._pow }
    }
    class goblin extends unit {
        get symbol() { return 'G' }
        get attackPow() { return 3 }
    }
    class wall {
        pos: pos
        unit: null = null
    ...
    class map {
        ...
        elvesCount(): number {
            return this.elves.filter(e => e.health > 0).length
        }

        constructor(data: string[], elfPow: number) {
    ...
    while (!win) {
        pow++;
        let m = new map(data, pow)
        let start = m.elvesCount();
        //console.log(m.show() + '\n')
        m.run()
        let end = m.elvesCount();

        win = start === end
        console.log('pow', pow, 'gives', start, '->', end)
        //console.log(m.show() + '\n')
        if(win){
            return m.outcome()
        }
